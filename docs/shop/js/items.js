...
function renderProductCard(grid, path, prodData) {
    ...
    const link = hasCustomURL ?
        prodData.url :
        `product.html?path=${path}&id=${p.id}`;
    ...
    <a href="${link}" ${hasCustomURL ? 'target="_blank"' : ''}>
      ...
      ${p.sale_price < p.price ? `<div class="item-price"><s>${p.price + p.shipping}</s> <b>${p.total}</b>€+</div>` : `<div class="item-price">${p.total}€+</div>`}
    </a>
  `;
    grid.appendChild(card);
}
...
async function showItems() {
    ...
    if (footer) {
        ...
        if (isTopLevel) {
            backBtn.href = '/shop/shop.html';
            backBtn.textContent = '← Back to Shop Home Page';
        } else {
            const parentPath = segments.slice(0, -1).join('/');
            backBtn.href = `items.html?path=${parentPath}`;
